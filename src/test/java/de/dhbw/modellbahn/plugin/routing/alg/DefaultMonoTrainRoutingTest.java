...
//        showRoutingGraph(oldRoutingGraph);
        ...
        try {
            ...
            List<DirectedNode> path = routing.findShortestPath(eIn, cIn);
    ...
    void testIsRoutingWorkingAtAll2() throws PathNotPossibleException {
        ...
        // action
        DefaultMonoTrainRouting routing = new DefaultMonoTrainRouting(graph);
        List<DirectedNode> shortestPath = routing.findShortestPath(pointAIn, pointCIn);
    ...
    @Test
    void testPathIsCorrectlyMapped() throws PathNotPossibleException {
        ...
        DefaultMonoTrainRouting routing = new DefaultMonoTrainRouting(graph);
        List<DirectedNode> graphPath = routing.findShortestPath(eIn, cIn);

        // TODO test with Mapping of WeightedDistanceEdges
...
//        assertThat(graphPath.getWeight()).isEqualTo(40.0);
    ...
    void findShortestPath3(DirectedNode source, DirectedNode destination, List<DirectedNode> expected) {
        ...
        try {
            path = routing.findShortestPath(source, destination);
        } catch (PathNotPossibleException e) {
            e.printStackTrace();
        }
